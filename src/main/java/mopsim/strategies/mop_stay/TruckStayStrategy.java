...
public class TruckStayStrategy implements MOPStayStrategy {
	...
	public final static int LONG_SHORT_BOUND2 = 100000;
	...
	public double nextStayLength(double currentTime, double departureTime) {
		...
		if (currentTime < LONG_SHORT_BOUND || currentTime > LONG_SHORT_BOUND2 ||
				(currentTime - departureTime) < LONG_SHORT_TRAVEL_TIME || rand.nextDouble() < 0.7) {
			while (true) {
